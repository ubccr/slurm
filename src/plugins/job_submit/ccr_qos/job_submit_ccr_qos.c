...
extern int job_submit(struct job_descriptor *job_desc, uint32_t submit_uid, char **err_msg) {
	...
	if (job_desc->qos){
		...
		xstrcat(job_desc->qos, job_desc->partition);
                ...
                if (err_msg)
                    *err_msg = xstrdup("Valid QOS required in batch script.  Usually this is the same as the partition name specified.  See CCR documentation for more info. Jobs without valid QoS will fail to run after June 1, 2018.");
	}
